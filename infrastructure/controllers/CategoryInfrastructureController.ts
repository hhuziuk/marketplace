...
class CategoryInfrastructureController {
    ...
    async getAll(req: Request, res: Response, next: NextFunction){
        try{
            const categories = await CategoryInfrastructureService.getAll();
    ...
    async getById(req: Request, res: Response, next: NextFunction){
        try{
            const {categorykId} = req.params;
            if (!categorykId) {
                throw ApiError.BadRequest(`Required data is missing`);
            }
            const category = await CategoryInfrastructureService.getById(categorykId);
            return res.json(category)
        } catch(e){
            next(e)
            logger.error(e)
