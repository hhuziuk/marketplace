...
class PaymentInfrastructureService {
    ...
    async createPayment(amount: number, method: PaymentMethod, userId: string, orderId: string): Promise<void> {
        try {
            ...
            if (!orderExists || !userExists) {
                ApiError.NotFound(`Order or User not found`);
            }
            ...
            ApiError.InternalServerError('Error creating payment');
        }
    }
        ...
        try {
            ...
            switch (method) {
                ...
                case PaymentMethod.BankTransfer:
                    ...
                    break;
                default:
                    ApiError.BadRequest(`Payment method ${method} is not supported`);
            }

            await this.createPayment(amount, method, userId, orderId);
        } catch (error) {
            logger.error('Error processing payment:', error);
            ApiError.InternalServerError('Error processing payment');
        }
    }
    ...
    async setPaymentMethod(paymentId: string, method: PaymentMethod): Promise<void> {
        ...
        if (!payment) {
            ApiError.BadRequest(`Payment with id ${paymentId} not found`);
        }
    ...
    async payByCard(amount: number, paymentMethodId: string): Promise<string> {
        try {
            ...
            logger.info('Payment successful:', paymentIntent);
